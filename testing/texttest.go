...
func main() {
	//file, err := gas.Abs("github.com/golang/freetype/testdata/luxisr.ttf")
	//file := "C:\\Users\\Hanavan\\go\\src\\github.com\\golang\\freetype\\testdata\\luxisr.ttf"
	file := "C:\\Windows\\Fonts\\times.ttf"
	/*if err != nil {
		fmt.Printf("Find font file: %v", err)
		return
	}*/
	fmt.Printf("Starting\n")
	window, err := initGL()
	if err != nil {
		fmt.Printf("InitGL: %v", err)
		return
	}

	defer glfw.Terminate()

	fmt.Printf("Loading fonts\n")
	// Load the same font at different scale factors and directions.

	for i := range fonts {
		fonts[i], err = loadFont(file, int32(12+i))
		if err != nil {
			fmt.Printf("LoadFont: %v", err)
			return
		}
...
func loadFont(file string, scale int32) (*gltext.Font, error) {
	...
	fmt.Printf("gltext.LoadTruetype()\n")
...
func initGL() (*glfw.Window, error) {
	...
	//window.SetWindowTitle("go-gl/gltext: Truetype font example")
	...
	fmt.Printf("window.MakeContextCurrent()\n")
